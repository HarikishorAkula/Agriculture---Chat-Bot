...
for k, v in [
    ...
    ("api_calls", 0),
    ("cache_hits", 0),
    ("gemini_failed", False),         # auto-set when Gemini quota hits
    ("force_pinecone_only", False),   # user manually switched
]:
...
def get_answer_pinecone_only(query):
    """Return raw document text from Pinecone. No LLM involved."""
...
def get_answer_gemini(query):
    """Query Pinecone for context then ask Gemini. Falls back on quota error."""
    key = hashlib.md5(query.strip().lower().encode()).hexdigest()

    if key in st.session_state.answer_cache:
        return st.session_state.answer_cache[key], True, False  # answer, cached, gemini_failed
...
"""
    ...
    try:
        ...
        if any(x in err_str for x in ["quota", "429", "resource_exhausted", "rate limit", "billing"]):
            st.session_state.gemini_failed = True   # auto-switch to pinecone only
...
if not st.session_state.chat_history:
    ...
    for item in reversed(st.session_state.chat_history):
        # Support both old 3-tuple and new 4-tuple history entries
        if len(item) == 4:
